...
def test_no_exp_filter(client: Client):
    ...
    qs = "?SERVICE=ATLAS&REQUEST=GetPrint&MAP={}&TEMPLATE=layout1-atlas".format(PROJECT_ATLAS_SIMPLE)
    rv = client.get(qs, PROJECT_ATLAS_SIMPLE)
    assert rv.status_code == 400
    assert rv.headers.get("Content-Type", "").find("application/json") == 0
    b = json.loads(rv.content.decode("utf-8"))
...
def test_invalid_exp_filter(client: Client):
    ...
    qs = "?SERVICE=ATLAS&REQUEST=GetPrint&MAP={}&TEMPLATE=layout1-atlas&EXP_FILTER=id in (1, 2".format(
        PROJECT_ATLAS_SIMPLE
    )
    rv = client.get(qs, PROJECT_ATLAS_SIMPLE)
    assert rv.status_code == 400
    assert rv.headers.get("Content-Type", "").find("application/json") == 0
    ...
    if Qgis.versionInt() >= 34200:
        message += (
            "Incomplete expression. You might not have finished the full expression., expecting COMMA or ')'"
        )
    else:
...
def test_invalid_exp_filter_field(client: Client):
    ...
    qs = "?SERVICE=ATLAS&REQUEST=GetPrint&MAP={}&TEMPLATE=layout1-atlas&EXP_FILTER=fakeId in (1, 2)".format(
        PROJECT_ATLAS_SIMPLE
    )
    rv = client.get(qs, PROJECT_ATLAS_SIMPLE)
    assert rv.status_code == 400
...
def test_invalid_template(client: Client):
    ...
    qs = "?SERVICE=ATLAS&REQUEST=GetPrint&MAP={}&TEMPLATE=Fakelayout1-atlas&EXP_FILTER=id in (1, 2)".format(
        PROJECT_ATLAS_SIMPLE
    )
    rv = client.get(qs, PROJECT_ATLAS_SIMPLE)
    assert rv.status_code == 400
...
def test_valid_getprint_report(client: Client, output_dir: Path):
    ...
    qs = "?SERVICE=ATLAS&REQUEST=GetPrint&MAP={}&TEMPLATE=layout2-report&".format(PROJECT_ATLAS_SIMPLE)
    rv = client.get(qs, PROJECT_ATLAS_SIMPLE)
    assert rv.status_code == 200
